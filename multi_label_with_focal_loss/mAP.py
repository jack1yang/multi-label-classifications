...
if __name__=='__main__':
    ...
    predictions_data = json.load(open('predictions_frontal_train224.json'))
    gts_data = json.load(open('gts_frontal_train224.json'))
    predictions = []
    gts = []
    for img_name, prob in predictions_data.items():
        predictions.append(prob)
        gts.append(gts_data[img_name])
    predictions = np.stack(predictions, 0)#.reshape(-1, 82)
    gts = np.stack(gts, 0)#.reshape(-1, 82)
