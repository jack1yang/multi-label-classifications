...
class EncoderCNN_singleimage_frontal(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv_cls = nn.Conv2d(in_channels = 2048, out_channels = num_classes, kernel_size=1)

        self.pool_mil = nn.MaxPool2d(7, stride = 0)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(0.5)
        # self.init_weights()
    ...
    def forward( self, images ):
        ...
        x0 = self.conv_cls( images ) #(bacth_size, num_classes, 7, 7)
        x = self.pool_mil(x0) # (batch_size, num_classes, 1, 1)
        x = x.squeeze(2).squeeze(2)
        x1 = torch.add(torch.mul(x0.view(x.size(0), 82, -1), -1), 1)
        cumprod = torch.cumprod(x1, 2) #(batch_size, 82, 7*7)
        #out = torch.min(x, torch.add(torch.mul(cumprod[:, :, -1], -1), 1))
        out = torch.add(torch.mul(cumprod[:, :, -1], -1), 1)
...
class EncoderCNN_singleimage_lateral(nn.Module):
    def __init__(self, num_classes):
        ...
        resnet = models.resnet152()
        modules = list( resnet.children() )[ :-2 ] # delete the last fc layer and avg pool.
        resnet_conv = nn.Sequential( *modules ) # last conv feature (batch_size, 7,7 ,2048)
        #self.avgpool = nn.AvgPool2d( 7 )
    ...
    def forward( self, images ):
        ...
        # Last conv layer feature map
        images = self.resnet_conv_lateral( images ) #(batch_size, 2048, 7, 7)
        x0 = self.conv_cls( images ) #(bacth_size, num_classes, 7, 7)
        x = self.pool_mil(x0) # (batch_size, num_classes, 1, 1)
        x = x.squeeze(2).squeeze(2)
        x1 = torch.add(torch.mul(x0.view(x.size(0), 82, -1), -1), 1)
